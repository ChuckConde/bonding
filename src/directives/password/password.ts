import { AfterViewInit, Directive, ElementRef, forwardRef, Input, OnInit, Renderer2 } from '@angular/core';
...
export class PasswordDirective {
  ...
  /**
   * add element to input's parent,
   * add classes and
   * style the element to display to the right of input,
   * add a click event to chnage input type between text and password
   * @method ngAfterViewInit
   */
  public ngAfterViewInit(): void {
    const inputElem = this.elem.nativeElement.getElementsByClassName('text-input')[0];
    ...
    this.renderer.addClass(toggleElem, 'password-toggle-button');
    const strengthElem = this.renderer.createElement('div');
